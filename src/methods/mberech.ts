...
export const MBERECH = (
  {
    ...
    JLHINZU,
    ...INPUTDATA
  }: MBERECH_INPUT, CONFIG:ICONFIGURATION) => {
  let VFRB: Big;      // Verbrauchter Freibetrag bei Berechnung des laufenden Arbeitslohns, in Cent
  ...
  let LSTJAHR: Big;   // Jahreslohnsteuer in Euro
  ...
  const { ANP, KFB, ZTABFB, FVBZ:fvbz, KZTAB } = MZTABFB({
    ...INPUTDATA,
    ZRE4,
    ZVBEZ,
    FVBZ,
  ...
  VFRB = (ANP.add(FVB).add(fvbz)).mul(100);
  ...
  // LSTJAHR = ST * F;
  LSTJAHR = ST.mul(F).round(0, Big.roundDown);
