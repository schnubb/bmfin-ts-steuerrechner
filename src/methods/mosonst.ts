...
export const MOSONST = ({...INPUTDATA}:IINPUT, CONFIG:ICONFIGURATION) => {
  ...
  // ZRE4J = JRE4 / 100
  let zre4j = JRE4.div(100);
  // ZVBEZJ = JVBEZ / 100
  let zvbezj = JVBEZ.div(100);
  // JLFREIB = JFREIB / 100
  let jlfreib = JFREIB.div(100)
  // JLHINZU = JHINZU / 100
  let jlhinzu = JHINZU.div(100);
  // MRE4
  ...
  let zre4vp = ZRE4VP.minus(JRE4ENT.div(100));
  // MZTABFB
  const { ANP, FVBZ:fvbzNew, KZTAB, ZTABFB, KFB } = MZTABFB({
    ...INPUTDATA,
    ZRE4,
    ZVBEZ,
    FVBZ,
  ...
  const VFRBS1 = ANP.add(FVB).add(FVBZ).mul(100);
  // MLSTJAHR
